...
namespace PBRT {
    namespace SpherGeom {
        ...
        __device__ __host__ float CosPhi(glm::vec3 w) {
            float sinTheta = SinTheta(w);
            return (sinTheta == 0.0f) ? 1.0f : glm::clamp(w.x / sinTheta, -1.0f, 1.0f);
        }
        __device__ __host__ float SinPhi(glm::vec3 w) {
            float sinTheta = SinTheta(w);
            return (sinTheta == 0.0f) ? 0.0f : glm::clamp(w.y / sinTheta, -1.0f, 1.0f);
        }

        __device__ __host__ bool SameHemisphere(glm::vec3 w, glm::vec3 wp) {
            return w.z * wp.z > 0.0f;
        }

        __device__ __host__ glm::vec3 SphericalDirection(float sinTheta, float cosTheta, float phi) {
